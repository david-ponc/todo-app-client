...
function Navbar({ links, navbarButton, user }) {
	...
	const sendToJoin = () =>
		router.push(navbarButton.path, '/join', { locale: `${router.locale}` });
			...
			<NavbarStyled>
				...
				{visibleMenu && (
					<NavStyled>
						...
						{!user && (
							<Button color='primary' design='full' onClick={sendToJoin}>
								{navbarButton.name}
							</Button>
						)}
						{user && (
							<ItemOptionTheme>
								Logout <HiLogout size={24} onClick={logout} cursor='pointer' />
							</ItemOptionTheme>
						)}
	...
	return (
		<NavbarStyled>
			...
			<NavStyled>
				...
				{!user && (
					<Button color='primary' design='compact' onClick={sendToJoin}>
						{navbarButton.name}
					</Button>
				)}
