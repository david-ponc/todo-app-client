	...
	const handleSubmit = async e => {
		if (e.key === 'Enter') {
			...
			if (value !== '' && category !== '') {
				...
				const { hasExistedTask, task } = await createTask(authToken, {
					content: value,
					category
				});
				if (hasExistedTask) {
				...
				!value
					? setError('El contenido de la tarea no es válido')
					: setError('Debe seleccionar una categoría para crear una tarea');
			}
		}
	};
			...
			<Wrapper error={error}>
				<FieldContainer>
					...
					{!content && !isMobileView && (
						<PlaceholderStyled>
							{placeholder[0]} <EnterCommand /> {placeholder[1]}
						</PlaceholderStyled>
					)}
					{!content && isMobileView && (
						<PlaceholderStyled>
							<EnterCommand /> {placeholder[1]}
						</PlaceholderStyled>
					)}
				</FieldContainer>
