...
async function confirmAccount(token) {
	const confirmResponse = await fetch(
		`${process.env.URL_API}/users/auth/confirm-email`,
		{
			method: 'PUT',
			headers: {
				'email-token': token
...
async function getUser(identifier, token) {
	const userResponse = await fetch(
		`${process.env.URL_API}/users/${identifier}/profile`,
		{
			method: 'GET',
			headers: {
...
async function getTasksOfCurrentUser(identifier, token) {
	const tasksResponse = await fetch(
		`${process.env.URL_API}/tasks/${identifier}/list`,
		{
			method: 'GET',
			headers: {
				Authorization: `Bearer ${token}`
...
async function createTask(jwt, taskDraft) {
	...
	const createResponse = await fetch(
		`${process.env.URL_API}/tasks/${identifier}`,
		{
			method: 'POST',
			headers: {
